...
} Feature6_ImageVertex;

typedef struct {
    float4 position [[position]];
    float2 texCoord;
} Feature6_ImageColorInOut;

// Convert from YCbCr to rgb.
float4 Feature6_ycbcrToRGBTransform(float4 y, float4 CbCr) {
...
} Feature6_FogColorInOut;

// Fog the image vertex function.
vertex Feature6_FogColorInOut Feature6_fogVertexTransform(const device Feature6_FogVertex* cameraVertices [[ buffer(0) ]],
                                                         const device Feature6_FogVertex* sceneVertices [[ buffer(1) ]],
                                                         unsigned int vid [[ vertex_id ]]) {
    Feature6_FogColorInOut out;

    const device Feature6_FogVertex& cv = cameraVertices[vid];
    const device Feature6_FogVertex& sv = sceneVertices[vid];
...
fragment half4 Feature6_fogFragmentShader(Feature6_FogColorInOut in [[ stage_in ]],
                                 ...
                                 texture2d<uint> arDepthConfidence [[ texture(3) ]])
{
    ...
    float4 rgb = Feature6_ycbcrToRGBTransform(
    ...
    float4 rgb2 = Feature6_ycbcrToRGBTransform(float4(sum, 1.0), float4(sum2, 1.0));
