...
protocol Feature4_RenderDestinationProvider {
...
private let kAlignedInstanceUniformsSize: Int = ((MemoryLayout<InstanceUniforms>.size * kMaxAnchorInstanceCount) & ~0xFF) + 0x100
...
class Feature4_Renderer {
    let session: ARSession
    let matteGenerator: ARMatteGenerator
    let device: MTLDevice
    let inFlightSemaphore = DispatchSemaphore(value: kMaxBuffersInFlight)
    var renderDestination: Feature4_RenderDestinationProvider
    ...
    var typeFlag: Float = 0.0

    init(session: ARSession, metalDevice device: MTLDevice, renderDestination: Feature4_RenderDestinationProvider) {
    ...
    func loadMetal() {
        ...
        let capturedImageVertexFunction = defaultLibrary.makeFunction(name: "Feature4_capturedImageVertexTransform")!
        let capturedImageFragmentFunction = defaultLibrary.makeFunction(name: "Feature4_capturedImageFragmentShader")!
        ...
        let anchorGeometryVertexFunction = defaultLibrary.makeFunction(name: "Feature4_anchorGeometryVertexTransform")!
        let anchorGeometryFragmentFunction = defaultLibrary.makeFunction(name: "Feature4_anchorGeometryFragmentLighting")!
        ...
        let compositeImageVertexFunction = defaultLibrary.makeFunction(name: "Feature4_compositeImageVertexTransform")!
        let compositeImageFragmentFunction = defaultLibrary.makeFunction(name: "Feature4_compositeImageFragmentShader")!
    ...
    func loadAssets() {
        guard geometryVertexDescriptor != nil else { return }
    ...
    private func loadTexture() {
        ...
        bgTexture = try! textureLoader.newTexture(
            name: "feature4",
            scaleFactor: 2.0,
            bundle: nil)
    }
    ...
    func compositeImagesWithEncoder(renderEncoder: MTLRenderCommandEncoder) {
        ...
        renderEncoder.setFragmentTexture(bgTexture, index: 6)

        // リファクタ推奨です
        let typeFlagBuffer = device.makeBuffer(length: MemoryLayout<Float>.size, options: [])!
        let vData = typeFlagBuffer.contents().bindMemory(to: Float.self, capacity: 1 / MemoryLayout<Float>.stride)
        vData[0] = typeFlag
        renderEncoder.setFragmentBuffer(typeFlagBuffer, offset: 0, index: 0)
